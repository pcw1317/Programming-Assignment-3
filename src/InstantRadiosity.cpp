...
void InstantRadiosityEmbree::addMesh( host_mesh_t mesh )
{
...
std::vector<LightData> InstantRadiosityEmbree::getVPLpos( LightData light, unsigned int rayCount, unsigned int recursionDepth )
{
    ...
    if( recursionDepth > 0 )
    {
        for( unsigned int rayIter = 0; rayIter < rayCount; ++rayIter )
        {
            ...
            // Ray init.
            RTCRay ray;
            ray.org[0] = light.position.x;
            ray.org[1] = light.position.y;
            ray.org[2] = light.position.z;
            ray.dir[0] = randomDir.x;
            ray.dir[1] = randomDir.y;
            ray.dir[2] = randomDir.z;
            ray.tnear = 0.f;
            ray.tfar = INFINITY;
...
std::vector<LightData> InstantRadiosityEmbree::getVPLpos( AreaLightData light, unsigned int sampleCount, unsigned int rayCount, unsigned int recursionDepth )
{
    ...
    for( unsigned int sampleIter = 0; sampleIter < sampleCount; ++sampleIter )
    {
